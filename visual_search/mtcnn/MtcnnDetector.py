...
matplotlib.use("Pdf")
import cv2
caffe_root = '/home/caffe-face-caffe-face/install/python/'  #change to your caffe root path
import sys
...
class FaceDetector_mt(object):
    ...
    def __init__(self,
        ...
        model_P = '/root/PycharmProjects/image_spark/mtcnn/model/det1.prototxt'
        weights_P = '/root/PycharmProjects/image_spark/mtcnn/model/det1.caffemodel'
        model_R = '/root/PycharmProjects/image_spark/mtcnn/model/det2.prototxt'
        weights_R = '/root/PycharmProjects/image_spark/mtcnn/model/det2.caffemodel'
        model_O = '/root/PycharmProjects/image_spark/mtcnn/model/det3.prototxt'
        weights_O = '/root/PycharmProjects/image_spark/mtcnn/model/det3.caffemodel'
        weights_cl = '/home/mtcnn-cpp-master/face_example/data/face_model.caffemodel'
        model_cl = '/home/mtcnn-cpp-master/face_example/face_deploy.prototxt'
        model_lt = "/home/mtcnn-cpp-master/Light_cnn/lightend_cnn/face_verification_experiment-master/proto/LightenedCNN_CS_deploy.prototxt"
        weights_lt = '/home/mtcnn-cpp-master/Light_cnn/lightend_cnn/face_verification_experiment-master/model/LightenedCNN_C.caffemodel'
        ...
        self.clnet = caffe.Net(model_cl,weights_cl,caffe.TEST)
        self.ltnet = caffe.Net(model_lt,weights_lt,caffe.TEST)
    ...
    def recognise_cl(self,img):
        ...
        im_in2 = im_in2[None, :]
        im_in2 = im_in2.transpose((0, 3, 1, 2))
        rep1 = self.clnet.forward_all(data = im_in)
        feature1 = np.float32(self.clnet.blobs['fc5'].data[0].flatten())
        rep2 = self.clnet.forward_all(data = im_in2)
        feature2 = np.float32(self.clnet.blobs['fc5'].data[0].flatten())
        return feature1.tolist() + feature2.tolist()

    def recognise_lt(self,img):
        GrayImg = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        GrayImg = np.float32(GrayImg)/255
        im_in = cv2.resize(GrayImg, (128, 128), interpolation=cv2.INTER_NEAREST)
        #self.clnet.blobs['data'].reshape(1, 3, 112, 96)
        rep1 = self.ltnet.forward_all(data = im_in)
        feature1 = np.float32(self.ltnet.blobs['eltwise_fc1'].data[0].flatten())
        return feature1.tolist()

    def recognise_comb(self,img):
        feature1 = self.recognise_cl(img)
        feature2 = self.recognise_lt(img)
